...
task :deploy => :configure_kubectl do
  ENV["S3_BUCKET"] = "gpii-kubernetes-state"
  ENV["KOPS_STATE_STORE"] = "s3://#{ENV['S3_BUCKET']}"
  sh "kubectl create -f https://raw.githubusercontent.com/kubernetes/kops/master/addons/kubernetes-dashboard/v1.5.0.yaml"
  sh "kubectl create -f ../deploy_gpii/couchdb-deploy.yml"
  sh "kubectl create -f ../deploy_gpii/couchdb-svc.yml"
  sh "kubectl create -f ../deploy_gpii/dataloader-job.yml"
  sh "kubectl create -f ../deploy_gpii/preferences-deploy.yml"
  sh "kubectl create -f ../deploy_gpii/preferences-svc.yml"
  sh "kubectl create -f ../deploy_gpii/flowmanager-deploy.yml"
  sh "kubectl create -f ../deploy_gpii/flowmanager-svc.yml"
end
