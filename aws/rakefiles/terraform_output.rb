...
def terraform_output_runner(dir, tmpdir, &block)
  ...
  if terraform_cmd_ok != 0
    ok = false
    msg = "terraform_output failed with return code #{terraform_cmd_ok}. Terraform output was:\n#{terraform_cmd_output}\n"
    block.call(ok, msg)
    return
  end
  ...
  if jq_ok == 0
    ok = true
    msg = JSON.parse(jq_cmd_output)
    block.call(ok, msg)
  else
    ok = false
    msg = "terraform_output failed with return code #{jq_ok}. Terraform output was:\n#{terraform_cmd_output}\njq output was:\n#{jq_cmd_output}\n"
    block.call(ok, msg)
  end
end
